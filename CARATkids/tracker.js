(function () {
    ...
    (async function initFirebase() {
        try {
            ...
            // Envia logs acumulados antes de a SDK carregar
    ...
    async function sendToFirebase(data) {
        if (!firebaseInitialized) {
            pendingLogs.push(data); // Guarda na fila se o Firebase ainda não estiver pronto
            return;
        }
        try {
            const { collection, addDoc, db } = window.firebaseDB;
            await addDoc(collection(db, "interactions"), data);
            console.log("Gravado no Firebase!");
        } catch (error) {
    ...
    function regEvent(eventType, eventDetails) {
        ...
        sendToFirebase(eventData); // Envia automaticamente para a base de dados
    }
